...
def evaluate(agent: ShoppingAgent, dataset_path: str | Path) -> EvaluationResult:
    ...
    retrieval_hits = tool_hits = constraint_hits = parameter_hits = 0
    for case in cases:
        response = agent.run(AgentRequest.model_validate(case["request"]))
        returned_ids = {hit.product.id for hit in response.hits}
        retrieval_hits += bool(returned_ids.intersection(case.get("relevant_ids", [])))
    ...
    return EvaluationResult(len(cases), retrieval_hits / total, tool_hits / total, constraint_hits / total, parameter_hits / total)
